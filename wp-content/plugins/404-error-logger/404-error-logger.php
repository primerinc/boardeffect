...
class Log_404 {
	...
	private function supported_cache_plugin_active(){
		return ( defined('W3TC') || defined('WPCACHEHOME') || class_exists('HyperCache') );
	}
...
<?php
	...
	function log_404s () {
		...
		// pop old entries if we exceeded the limit
		$max = intval( $this->options['max_entries'] );
		$count = intval( $wpdb->get_var( "SELECT COUNT(*) FROM $this->table" ) );
		if( $count > $max ) {
			$extra = intval( $count - $max );
			$wpdb->query("DELETE FROM $this->table ORDER BY id ASC LIMIT $extra");
		}
	}
